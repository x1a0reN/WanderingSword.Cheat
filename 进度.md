...
| 21 | 防御 | 滑块 | ✅ 已创建 | |
| 22 | 暴击 | 滑块 | ✅ 已创建 | |
| 23 | 暴击抵抗 | 滑块 | ✅ 已补齐(UI)| 进度规划有但代码遗漏 |
| 24 | 闪避 | 滑块 | ✅ 已创建 | |
| 25 | 命中 | 滑块 | ✅ 已创建 | |
| 26 | 移动 | 滑块 | ✅ 已补齐(UI)| CT有，战斗格子移动力 |
| 27 | 集气速率 | 滑块 | ✅ 已补齐(UI)| CT有 |
| 28 | 气血/真气恢复速率 | 滑块×4 | ✅ 已补齐(UI)| CT有，4个恢复/回复速率 |
| 29 | 真气护盾 | 滑块 | ✅ 已补齐(UI)| CT有 |
| 30 | 名声 | 滑块 | ✅ 已补齐(UI)| CT有 |
| 31 | 气血护盾 | 滑块 | ✅ 已补齐(UI)| CT有 |
| 32 | 暴击伤害百分率 | 滑块 | ✅ 已补齐(UI)| CT有 |
| 33 | 金钱倍率 | 滑块 | ✅ 已补齐(UI)| CT有 |
| 34 | 武学点倍率 | 滑块 | ✅ 已补齐(UI)| CT有 |
| 35 | 真气消耗倍率 | 滑块 | ✅ 已补齐(UI)| CT有，可考虑放战斗Tab |
| — | *武器精通* | | | |
| 36 | 拳掌精通 | 滑块 | ✅ 已创建 | |
| 37 | 拳掌经验 | 滑块 | ✅ 已补齐(UI)| 进度规划有但代码只做了精通 |
| 38 | 剑法精通 | 滑块 | ✅ 已创建 | |
| 39 | 剑法经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 40 | 刀法精通 | 滑块 | ✅ 已创建 | |
| 41 | 刀法经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 42 | 枪棍精通 | 滑块 | ✅ 已创建 | |
| 43 | 枪棍经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 44 | 暗器精通 | 滑块 | ✅ 已创建 | |
| 45 | 暗器经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 46 | 其他武器精通 | 滑块 | ✅ 已补齐(UI)| 进度规划有但代码遗漏 |
| 47 | 其他武器经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
...
| 7 | 家园随时收获 | 开关 | ✅ 已创建 | |
| — | *生活精通* | | | |
| 8 | 锻造精通 | 滑块 | ✅ 已补齐(UI)| 进度规划有但代码未实现 |
| 9 | 锻造经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 10 | 医术精通 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 11 | 医术经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 12 | 制衣精通 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 13 | 制衣经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 14 | 炼丹精通 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 15 | 炼丹经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 16 | 烹饪精通 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 17 | 烹饪经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 18 | 采集精通 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 19 | 采集经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 20 | 钓鱼精通 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 21 | 钓鱼经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 22 | 饮酒精通 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 23 | 饮酒经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 24 | 茶道精通 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 25 | 茶道经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 26 | 口才精通 | 滑块 | ✅ 已补齐(UI)| CT有，进度.md遗漏 |
| 27 | 口才经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
| 28 | 书法精通 | 滑块 | ✅ 已补齐(UI)| CT有，进度.md遗漏 |
| 29 | 书法经验 | 滑块 | ✅ 已补齐(UI)| 同上 |
...
| 8 | 强制显示NPC互动 | 开关 | ✅ 已创建 | |
| 9 | 物品质量 (送礼) | 下拉框 | ✅ 已补齐(UI)| 进度规划有但代码遗漏 |
...
| 13 | 解锁全图鉴 | 开关 | ✅ 已创建 | |
| 14 | 解锁全成就 | 开关 | ✅ 已创建 | |
| 15 | 世界移动速度 | 滑块 | ✅ 已补齐(UI)| CT有，大地图移动速度 |
| 16 | 场景移动速度 | 滑块 | ✅ 已补齐(UI)| CT有，场景内移动速度 |
| 17 | 屏幕设置 | 多项 | ✅ 已补齐(UI)| CT有，分辨率/VSync等6子项，复杂度高 |
| 18 | 开档难度系数 | 多项 | ✅ 已补齐(UI)| CT有，4难度×14参数，结构复杂 |
| 19 | 称号战力门槛 | 多项 | ✅ 已补齐(UI)| CT有，11个称号阈值 |
...
### 8. UComboBoxString 下拉框选项检测问题
- **问题**: 角色选择下拉框选中选项后无法检测到变化
- **原因**: UE4 SDK 中 `UComboBoxString` 类使用 `AddOption()` 添加选项时，选项存储在 `Options` 数组而非 `DefaultOptions` 数组。错误地使用 `DefaultOptions` 导致始终检测不到选项
- **错误尝试**: 直接访问 `Combo->Options` 数组 → 编译失败，SDK 中不存在该属性
- **解决**: 使用 SDK 提供的成员函数 `GetOptionCount()` 和 `GetSelectedIndex()` 获取下拉框状态
- **影响范围**: 此问题影响所有使用 `AddOption` 添加选项的下拉框（角色选择、门派等）
...
## 2026-03-01 UI 结构重构（数字编辑框版）

### 已完成

- 新增工厂函数 `CreateVolumeNumericEditBoxItem(...)`（与 `CreateVolumeEditBoxItem(...)` 同风格同布局）。
- `CreateVolumeEditBoxItem(...)` 字号已下调（更紧凑），统一用于数值输入视觉风格。
- 数字编辑框默认走 `KeyboardType = EVirtualKeyboardType::Number`，用于约束“数值类输入”场景。
- Tab0~Tab7 已按“大功能 -> 小功能”改为 `CreateCollapsiblePanel` 分组结构，不再扁平堆叠。
- 规则落地：数值修改优先数字编辑框；仅倍率/倍数/速度这类范围值保留滑块。
...
- 本轮仅完成 UI 架构与控件形态调整，业务逻辑绑定仍按后续 Phase 2 接入。
- “数字框禁止中文”已通过数字键盘类型约束输入场景；若需强制字符级过滤（非数字立即剔除），后续可加统一输入清洗轮询（仅对数字框集合生效）。
